...
28 & 1.02   & $10^{-5}$ & $10^{-5}$ & 0.1  & 128 & 200 & 200 & sparse & Solar Rossby\\ 
...
\pd{t} v + S x \pd{y} v + (S+f) u + \pd{y} p - B_{0} \pd{z} b_{y} &=& 0\\
...
\pd{t} b_{z} + S x \pd{y} b_{z} - B_{0} \pd{z} w &=& 0
\Eeq
...
\hat{W}(x) &=& \frac{i k \left(\hat{U}'(x)- \frac{ \left((f+S) \omega
   ...
   \Eeq
We can then substitute everything into equation (1) to get a second-order equation for $\hat{U}(x)$ of the general form 
\Beq
...
Rescale everything in terms of a bookkeeping parameter, $\eps$.
\Beq
S  \ \to \  - \frac{\pi ^2 B_{0}^2}{d^2 f} ( 1 + \eps^{2} R) 
\Eeq 
\Beq
...
\Beq
\Phi(x) \ = \ \frac{\pi^{2}}{d^{2}} + \frac{\epsilon ^2 \left(-B_{0}^4 k^4+B_{0}^2 f^2 q \left(k^2 R+q
...
This implies 
\Beq
\omega^{2} \ = \ \frac{q\,B_{0}^{2}}{(q+1)} \left[ k_z^2 \left(\frac{d^2}{\pi^{2}} k_z^2-R\right) \ - \ \frac{\left(6+\pi ^2\right) q+\pi
   ^2-6}{12 } \, k_{y}^{2}  \right] \ + \ ...
\Eeq
