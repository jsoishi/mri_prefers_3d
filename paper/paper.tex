...
\Eeq 
Generically $\gamma_{4} \ne 0$. We assume that $\gamma_{4} < 0$, but we do not know for sure if this is the case. The calculation is considerably more challenging than anything so far. If there is a negative 4th-order feedback, then 
\Beq
k_{y} \ \sim \ \sqrt{S_{2D}/S_{c} - \SSC}, 
\Eeq
which is consistent with Figure~\ref{fig:growth_rate}d near the critical point. Furthermore, a scenario where $\gamma_{4} > 0$ would require behaviour that is inconsistent with Figure~\ref{fig:growth_rate}d. 
