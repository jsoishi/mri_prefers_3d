...
We solve equations~(\ref{eq:ns})--~(\ref{eq:divu}) by assuming harmonic perturbations in $y$ and $z$, $f = \hat{f}(x) e^{i(k_y y + k_z) + \gamma t}$. This reduces to an eigenvalue problem in $x$, which we solve using the \emph{Dedalus} pseudospectral framework. The advantage of this particular form is that it can be expressed as a set of ten first-order equations with Dirichlet boundary conditions on all ten variables.
...
\begin{figure}[h]
  \includegraphics[width=0.33\textwidth]{run_11_output_growthrates.png}
  \includegraphics[width=0.33\textwidth]{run_12_output_growthrates.png}
  \includegraphics[width=0.33\textwidth]{run_13_output_growthrates.png}
...
Numerical simulations have consistently axisymmetric modes dominating early evolution of the MRI (cite Stone 95, etc) before parasitic modes cause a breakdown into MHD turbulence. However, our results recover this limit, as $\SSC$ becomes large. In this case

\begin{figure}[h]
  \includegraphics[width=\columnwidth]{alpha_vs_ssc.png}
  \caption{Angle $\alpha$ vs $\SSC$.}
  \label{fig:alpha}
\end{figure}
