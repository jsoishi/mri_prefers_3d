...
Two dimensional modes occur at $k_y = 0$ on the bottom of each figure.
  ...
  d) The phase angle, $\arctan(k_{y}/k_{z})$, as a function of $\SSC$ showing the exact transition to purely two-dimensional fastest modes at $\phi = 0$. }
...
\begin{figure}[h!]
  ...
  \caption{Velocity-magnetic field correlations relevant to the mean electromotive force, $(v_x B_y - v_y B_x)/|u| |B|$ for the eigenvectors of the fastest growing mode at $\SSC = 1.02$.}
...
We should expect the generation of a mean electromotive force in the linear regime, \textit{i.e}
...
Figure~\ref{fig:correlation} shows the correlation between $u$ and $b$ as a function of $x$ over the domain for $\SSC = 1.02$, proportional to the mean electromotive force.
The correlation is non-zero, demonstrating the tendency of the 3D MRI eigenmodes to act as a mean-field dynamo.
