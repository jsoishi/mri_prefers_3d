...
To run using 4 processes, you would use:
    $ mpiexec -n 4 python3 mri.py 1.20

This script will pull global parameters from an mri_options.config file.

The parameters corresponding to each output file are written to a
data_parameters.csv file that is generated by the script if it doesn't
already exist.
...
# Parse .config file to set global parameters for script
config = ConfigParser()
config.read('mri_options.config')
...
kx     =  eval(config.get('parameters','kx'))
S      = eval(config.get('parameters','S'))
f      =  eval(config.get('parameters','f'))
cutoff =  eval(config.get('parameters','cutoff'))
...
if CW.rank == 0:
    ...
    for file in os.scandir():
        if file.name.endswith('.h5'):
            if int(file.name[12:16]) == newest:
                newest = int(file.name[12:16]) + 1
    newest = str(newest)
    while len(newest) < 4:
        newest = '0' + newest
    datafile = h5py.File('mri_dataset_%s.h5'%newest, 'w')
    ...
    if config.getboolean('output','eigvec') == True:
        dset = datafile.create_dataset('eigvec',data=eigvec_global)
    datafile.close()