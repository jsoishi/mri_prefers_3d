...
fig, ax = plt.subplots()
ax.scatter(ssc, alpha, marker='o', c=growth, yunits=radians, zorder=2)#,edgecolor='k',linewidth=0.8)
...
a_thresh = 0.01
coefs = poly.polyfit(ssc[alpha > a_thresh], alpha[alpha > a_thresh], 1)
ffit = poly.polyval(ssc_new, coefs)
#ax.plot(ssc_new, ffit)

#axins = ax.inset_axes([0.7, 0.7, 0.24, 0.24])
#axins.scatter(ssc, alpha, marker='o',c=growth, yunits=radians)
...
ax.axvline(0.102,color='k', alpha=0.4)
ax.set_ylim(-0.1,1.0)
ax.set_xlim(0,4.1)
fig.tight_layout()
if use_opt:
    fig.savefig("plots/alpha_vs_ssc.pdf")
else:
    fig.savefig("plots/alpha_vs_ssc_grid.pdf")

# plot against R
ax.clear()
ax.scatter(R, alpha, marker='o',c=growth, yunits=radians)

axins = ax.inset_axes([0.7, 0.7, 0.24, 0.24])
axins.scatter(R, alpha, marker='o',c=growth, yunits=radians)
# sub region of the original image
...
ax.set_ylim(-0.1,1.0)
if use_opt:
    fig.savefig("plots/alpha_vs_R.pdf")
else:
    fig.savefig("plots/alpha_vs_R_grid.pdf")

    
# plot against (S/Sc)^2
ax.clear()
ssc2 = ssc**2
ax.scatter(ssc2, alpha, marker='o',c=growth, yunits=radians)

axins = ax.inset_axes([0.7, 0.7, 0.24, 0.24])
axins.scatter(ssc2, alpha, marker='o',c=growth, yunits=radians)
# sub region of the original image
x1, x2, y1, y2 = 1.4, 1.45, -0.05, 0.15
axins.set_xlim(x1, x2)
axins.set_ylim(y1, y2)
...
ax.set_ylim(-0.1,1.0)
if use_opt:
    fig.savefig("plots/alpha_vs_ssc2.pdf")
else:
    fig.savefig("plots/alpha_vs_ssc2_grid.pdf")
